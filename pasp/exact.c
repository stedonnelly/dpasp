...
static PyObject* exact(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  double *R = NULL;
  bool r = false, parallel = true, lstable_sat = true, quiet = false, status = true, verbose = false;
  const char *psem_arg = "credal";
  static char *kwlist[] = { "program", "parallel", "lstable_sat", "psemantics", "quiet", "status", "verbose", NULL };
  psemantics_t psem = CREDAL_SEMANTICS;

  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|bbsbbb", kwlist, &py_P, &parallel, &lstable_sat,
        &psem_arg, &quiet, &status, &verbose))
    return NULL;
  ...
  if (!exact_enum(&p, &R, lstable_sat, psem, quiet, status, verbose)) goto cleanup;
