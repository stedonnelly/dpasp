...
All metadata, dependencies, and package discovery live in pyproject.toml.
This file exists only because setuptools needs setup.py for ext_modules.
"""
...
def check_environment_vars(macros: list) -> list:
    _attribute_environment_var(macros, "NUM_PROCS")
    return macros


STD_MACROS = check_environment_vars(
    [("NUM_PROCS", str(nproc - 1 if (nproc := os.cpu_count()) > 1 else nproc)),
     ("_GNU_SOURCE", None)]
)
...
setup(ext_modules=[exact, ground, learn, sample, approx])
